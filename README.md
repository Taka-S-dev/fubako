...
- **参照**: 共有フォルダのパスや Wiki・チケットの URL など、フォルダに入れられない置き場所を作業ごとに何件でも登録できます。クリックで既定のアプリが開き（`https://` はブラウザ、`\\サーバー\共有` はエクスプローラー）、右クリックでコピーできます。表示名を付ければ長いパスも一覧で読めます。開く対象は http/https とドライブ・共有フォルダのパスに限っており、それ以外の形式は実行しません
...
これらの性質は Rust のユニットテスト41件で固定しており、CI で毎回検証しています（ファイルを開いたままリネームした場合の挙動も含む）。検索の絞り込みや工数入力の解釈といった画面側のロジックにも、別途38件のテストがあります。
...
| `Esc` | 絞り込み解除 → 選択解除 → トレイへ格納（モーダルが開いていれば先に閉じる） |
